...
export default function App() {
    ...
    try {
      ...
      try {
        ...
        if (response.ok) {
          ...
          await SecureStore.setItemAsync('deviceId', data.deviceId);
  ...
  const emailSignIn = async () => {
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        await SecureStore.setItemAsync('token', data.item.token);
        await SecureStore.setItemAsync('deviceId', data.item.deviceId);
        setUser({ email });
      } else {
        const err = await response.text();
